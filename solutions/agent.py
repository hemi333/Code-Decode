...
try:
    from solutions.tools import TOOLS, dispatch
except ImportError:
...
class Session:
    ...
    def ask(self, question: str, verbose: bool = True) -> str:
        ...
        # ------------------------------------------------------------------
        # ★ 새 질문을 기존 대화 뒤에 붙인다. 새로 만들지 않는 게 요점이다.
        ...
        for turn in range(1, MAX_TURNS + 1):
            ...
            for block in response.content:
                ...
                self.tool_calls[signature] += 1

                if self.tool_calls[signature] > MAX_REPEATS:
                    # 실행은 건너뛰지만 tool_result 는 반드시 만든다.
                    # 짝이 안 맞으면 API 가 400 을 돌려준다.
                    output = {
                        "error": (
                            f"같은 호출을 {MAX_REPEATS}번 넘게 반복했습니다. "
                            "다른 방법을 쓰거나, 지금까지 얻은 정보로 답하세요."
                        )
                    }
                else:
